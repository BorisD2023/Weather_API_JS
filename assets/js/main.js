...
const beaufortSkala = (value) => {
    ...
    if (value >= 32.7) return `Orkan ${value} m/s, [Windstärke: 2`;
};

// ! Wind direction Funktion
const directionDeg = (deg) => {
    if (deg >= 20 && deg <= 69.99) return `North East (${deg}°)`;
    if (deg >= 70 && deg <= 109.99) return `East (${deg}°)`;
    if (deg >= 110 && deg <= 159.99) return `South East (${deg}°)`;
    if (deg >= 160 && deg <= 199.99) return `South (${deg}°)`;
    if (deg >= 200 && deg <= 249.99) return `South West (${deg}°)`;
    if (deg >= 250 && deg <= 289.99) return `West (${deg}°)`;
    if (deg >= 290 && deg <= 339.99) return `North West (${deg}°)`;
    if (deg >= 340) return `North (${deg}°)`;
...
checkBtn.addEventListener("click", (event) => {
    ...
    localTimeArticle.childElementCount >= 1
    ...
    pressureArticle.childElementCount >= 1
        ? (pressureArticle.innerHTML = "")
        : "";
    humidityArticle.childElementCount >= 1
        ? (humidityArticle.innerHTML = "")
        : "";
    sunriseArticle.childElementCount >= 1
    ...
    fetch(
        ...
        .then((data) => {
            ...
            obtainedTag.textContent = `Obtained at ${getHours}:${getMinutes}, ${getDate} ${obtainedTime.toLocaleString(
                "default",
                { month: "short" }
            )} ${obtainedTime.getFullYear()}`;
            ...
            utcTime.setSeconds(
                utcTime.getSeconds() + shiftInSeconds + berlinOffset
            );
            ...
            localTime.textContent = `${getLocalHours}:${getLocalMinutes}, ${getLocalDate} ${utcTime.toLocaleString(
                "default",
                { month: "short" }
            )} ${utcTime.getFullYear()}`;
            ...
            imgWind.src =
                "https://media0.giphy.com/media/XmfjDw9rYnYhAbKtsa/giphy.gif?cid=ecf05e4796z17xqk2d2mt9xaqkwn62o6393xqzgsc4nj909z&rid=giphy.gif&ct=s";
            ...
            let getSunriseMinutes = new Date(
            ...
            imgSunrise.src =
                "https://media4.giphy.com/media/hsgoFoHbqNDaXkcVG5/giphy.gif?cid=ecf05e4707h9wbej8ac5srihf541k6ikvkhc5j7esdrmdc5l&rid=giphy.gif&ct=s";
            ...
            imgSunset.src =
                "https://media4.giphy.com/media/SWOrko6ftbH1bO5Vml/giphy.gif?cid=ecf05e470977i01c9ah0w6r17qzggzf0umi8z30i2fkyex0h&rid=giphy.gif&ct=s";
            ...
            geoTag.textContent = `[${latitude.toFixed(2)}, ${longitude.toFixed(
                2
            )}]`;
            geoArticle.appendChild(geoTag);
            localTimeArticle.style.boxShadow =
